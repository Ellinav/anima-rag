...
function loadSession(sessionId) {
  ...
  try {
    const safeId = sessionId.replace(/[^a-zA-Z0-9@\-\._\u4e00-\u9fa5]/g, "_");
    ...
    console.warn(`[Anima Session] Load failed for ${sessionId}: ${e.message}`);
    return { memories: [] };
  }
}
...
function saveSession(sessionId, data) {
  ...
  try {
    ...
    console.error(`[Anima Session] Save failed for ${sessionId}: ${e.message}`);
  }
}
...
function processEchoLogic(
  ...
  globalTop50.forEach((r) => {
    if (!freshScoreMap.has(r.item.id)) freshScoreMap.set(r.item.id, r.score);
  });
  ...
  currentResults.forEach((res) => {
    ...
    if (!nextMemories[resId]) {
      ...
      const isImportant = tags.some((t) => impTags.includes(t.toLowerCase()));
  ...
  try {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        if (!cleanMessage) cleanMessage = `HTTP Error ${response.status}`;
      }
...
async function performDynamicStrategy(indices, vector, config, ignoreIds = []) {
      ...
      if (importantStepConfig && importantStepConfig.labels) {
        ...
        importantStepConfig.labels.forEach((label) => {
          ...
          for (const res of importantPool) {
            if (countForThisLabel >= importantStepConfig.count) break;
            if (usedIds.has(res.item.id) || res.score < stepThreshold) continue;
      ...
      try {
        ...
        if (rerankResults && rerankResults.length > 0) {
          ...
          const rankedItems = rerankResults.map((r) => {
            const originalObj = documentsToSend[r.index].originalData;
        ...
        console.error(`[Anima Rerank] ❌ 重排失败，回退粗排逻辑:`, e.message);
        fallback();
      }
    } else {
      fallback();
  ...
  for (let i = 0; i < steps.length; i++) {
    ...
    for (const res of candidates) {
      ...
      if (res.score < globalMinScore) {
        if (["important", "status", "period", "special"].includes(step.type)) {
  ...
  finalResults.sort((a, b) => {
    ...
    const parseId = (str) => {
      ...
      return { batch: parseInt(parts[0] || 0), slice: parseInt(parts[1] || 0) };
    };
...
async function init(router) {
  ...
  router.post("/insert", async (req, res) => {
    // 1. 解构请求数据
    const { collectionId, text, tags, timestamp, apiConfig, index, batch_id } =
      req.body;
      ...
      await runInQueue(collectionId, async () => {
        ...
        if (index !== undefined && index !== null) {
          ...
          const duplicates = allItems.filter(
            (item) =>
              item.metadata && String(item.metadata.index) === String(index),
          );
          ...
          if (duplicates.length > 0) {
            ...
            for (const plan of deletionPlan) {
              try {
                ...
                console.warn(`[Anima] 覆盖清理旧文件失败: ${e.message}`);
              }
            }
          }
        }
      ...
      await runInQueue(collectionId, async () => {
        ...
        for (let i = 0; i < chunks.length; i++) {
          ...
          try {
            ...
            // 简单的进度日志
            if ((i + 1) % 10 === 0)
              console.log(`[Anima KB] 进度: ${i + 1}/${chunks.length}`);
          } catch (err) {
            console.error(`[Anima KB] 片段 ${i} 向量化失败:`, err.message);
          }
        }
      });
        ...
        indexItems.map(async (entry) => {
          try {
            ...
            if (!fs.existsSync(filePath)) {
              console.warn(`[Anima RAG] ⚠️ 文件丢失: ${fileName}`);
              return null;
            }
  ...
  router.post("/query", async (req, res) => {
    try {
      ...
      // 2. 向量化
      if (!searchText) return res.json({ chat_results: [], kb_results: [] });
      ...
      if (sessionId) {
        ...
        if (Array.isArray(loaded.memories)) {
          ...
          loaded.memories.forEach((m) => {
            if (m && m.item && m.item.id) lastMemories[m.item.id] = m;
          });
        } else {
      ...
      const chatTask = async () => {
        ...
        const rawIndices = (
          await Promise.all(
            targetIds.map((id) => getIndex(id, false).catch(() => null)),
          )
        ).filter((i) => i !== null);
        ...
        const safeIgnoreIds = Array.isArray(ignore_ids) ? ignore_ids : [];
        ...
        if (strat && strat.enabled) {
          ...
          // 简单模式
          const simpleCount =
            strat?.steps?.find((s) => s.type === "base")?.count || 5;
          ...
          raw = raw.filter((r) => r.score >= minScore).slice(0, simpleCount);
          raw.sort((a, b) => {
            const timeA = new Date(a.item.metadata.timestamp || 0).getTime();
            const timeB = new Date(b.item.metadata.timestamp || 0).getTime();
            return timeA - timeB;
          });
          return raw;
        }
      ...
      if (sessionId && chatContext.ids && chatContext.ids.length > 0) {
        try {
          console.log(`[Anima Echo] 🧠 启动回响处理... Session: ${sessionId}`);
          ...
          if (
            ...
            dynamicImpTags = chatContext.strategy.important.labels.map((t) =>
              t.toLowerCase(),
            );
            console.log(
          ...
          const { echoItems, nextMemories, echoLogs } = processEchoLogic(
            finalChatResults,
            globalTop50,
            lastMemories, // ✅ 传入
            finalEchoConfig,
          ...
          if (echoLogs && echoLogs.length > 0) {
            const formattedEchoLogs = echoLogs.map((l) => {
              const hasMeta = l.meta && typeof l.meta === "object";
        ...
        console.log(`[Anima Echo] ⚠️ 跳过回响 (无 SessionID 或 结果为空)`);
      }
  ...
  router.post("/import_collection", async (req, res) => {
    ...
    if (!collectionId || !zipData) return res.status(400).send("Missing data");
      ...
      await runInQueue(safeTargetName, async () => {
        ...
        for (const srcId of sourceIds) {
          try {
            ...
            const srcFolderPath = path.join(VECTOR_ROOT, safeSrcName);
            ...
            if (!sourceIndex) {
              console.warn(`[Anima Merge] ⚠️ 源库不存在，跳过: ${srcId}`);
              continue;
            }
            ...
            for (const item of items) {
              try {
                ...
                // B. 🔥 核心修复：必须从磁盘读取完整内容！
                if (!fs.existsSync(filePath)) {
                  console.warn(`[Anima Merge] ❌ 丢失物理文件: ${fileName}`);
                  failCount++;
                  continue;
                }
                ...
                const originalMetadata = fullData.metadata || fullData;
      ...
      await runInQueue(safeName, async () => {
        ...
        for (const item of items) {
          try {
            ...
            if (!fs.existsSync(filePath)) {
              console.warn(`[Anima Rebuild] ❌ 文件丢失: ${fileName}`);
              failCount++;
              continue;
            }
            ...
            if (!text) {
              console.warn(`[Anima Rebuild] ⚠️ 条目无文本，跳过: ${item.id}`);
              failCount++;
              continue;
            }
  ...
  router.post("/delete_collection", async (req, res) => {
    ...
    try {
      ...
      console.error(`[Anima RAG] Delete Collection Error: ${err.message}`);
      res.status(500).send(err.message);
    }
  });
  ...
  router.post("/delete_batch", async (req, res) => {
    ...
    await runInQueue(collectionId, async () => {
      ...
      const targets = allItems.filter(
        (item) =>
          item.metadata && String(item.metadata.batch_id) === String(batch_id),
      );

      if (targets.length === 0) {
        console.log(`[Anima RAG] Batch ${batch_id} 无旧数据，无需删除。`);
        ...
        try {
          ...
          if (plan.filePath) {
            if (fs.existsSync(plan.filePath)) {
              ...
              console.warn(`[Anima] ⚠️ 文件未找到 (跳过): ${plan.filePath}`);
            }
          }
        } catch (err) {
          console.error(
  ...
  router.post("/delete", async (req, res) => {
    ...
    await runInQueue(collectionId, async () => {
      ...
      if (targets.length === 0) {
        console.log(`[Anima RAG] 未找到 Index ${index} 的记录，跳过删除。`);
        res.json({ success: true, count: 0 });
        return;
      }
